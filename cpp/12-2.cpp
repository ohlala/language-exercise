...
//同11-3,私有的成员不能被继承,基类中的成员函数可以访问基类中的私有数据，
//但是派生类中的成员函数不可以，他只能访问自己增加的数据成员。有两种方式，
//一是通过基类的成员函数访问（11-3），二是将基类的数据成员定义为保护类型。
...
{
 public:
 	//要有构造函数，否则无法调用基类的构造函数。应是公共成员，否则主函数无法访问构造函数。
    Cat(char* name, int age, char* color)	
	:Pet(name, age, color){} 
    void GetInfo()
    {
        cout << "猫的名字:" << name << endl;	//这里要访问基类数据成员，故基类数据成员为保护类型 
        cout << "猫的年龄:" << age << endl;
